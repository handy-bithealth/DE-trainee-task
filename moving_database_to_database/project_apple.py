import psycopg2
import pyodbc
from config_project_apple import driver_sqlserver, server_sqlserver, database_sqlserver, uid_sqlserver, pwd_sqlserver
from config_project_apple import host_postgre, database_postgre, user_postgre, password_postgre, database_postgre_index
...
def checking_output_data():
    ...
    cursor_postgre.execute("SELECT number_of_rows from quantity_rows_table where table_name = 'apple_price_list'")
...
def checking_input_data():
    ...
    cursor_sqlserver.execute("SELECT max(pid) from apple_price_list")
...
def read_data_sqlserver(count_data_output):
    ...
    cursor_sqlserver.execute(f"SELECT pid, \
                model, price, country_code, country, \
                region, income_group, scraped_date \
                FROM dbo.apple_price_list \
                where pid > {count_data_output}")
...
def input_data_postgre(data_input):
    ...
    for i_data_input in data_input:
        cursor_postgre.execute('INSERT INTO apple_price_list( \
            pid, model, price, country_code, \
            country, region, income_group, scraped_date) \
            VALUES (%s, %s, %s, %s, %s, %s, %s, %s)' , i_data_input)
...
def update_index_table(count_data_output, inserted_rows):
    ...
    cursor_postgre.execute(f"UPDATE quantity_rows_table  SET number_of_rows = {inserted_rows_fix} WHERE table_name = 'apple_price_list';")
    conn_postgre.commit()
